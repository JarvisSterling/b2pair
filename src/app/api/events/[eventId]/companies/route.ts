...
export async function POST(
  ...
  const { name, contact_email, contact_name, capabilities, tier_id, booth_type, booth_number, team_limit, company_size, website } = body;
  ...
  const { data: member } = await supabase
    ...
    .insert({
      company_id: company.id,
      email: contact_email,
      name: contact_name || "",   // Use contact person's name, not company name
      role: "admin",
