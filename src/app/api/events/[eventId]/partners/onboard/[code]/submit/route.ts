...
/**
 * POST /api/events/[eventId]/partners/onboard/[code]/submit
 * Submit company for review — validates required fields, transitions status
 */
export async function POST(request: Request, { params }: Params) {
  ...
  // Get full company data for validation
  ...
  // Exhibitor-specific validation
  if (company.capabilities.includes("exhibitor")) {
    const ep = Array.isArray(company.exhibitor_profiles)
      ? company.exhibitor_profiles[0]
      : company.exhibitor_profiles;
    const hasProducts = ep?.products?.length > 0;
    const hasDescription = !!company.description_long;
