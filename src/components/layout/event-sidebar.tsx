...
export function EventSidebar({ workspaceId, eventId, workspaces, profile }: Props) {
  ...
  return (<>
    <aside className="hidden lg:flex w-64 flex-col border-r border-emerald-800 bg-emerald-950">
      {/* Workspace switcher */}
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active
                  ? "bg-emerald-900/60 border border-emerald-700 text-primary font-medium"
                  : "text-muted-foreground hover:bg-emerald-900/40 hover:text-foreground"
              )}
            >
    ...
    {menuOpen && (
      <>
        ...
        <aside className="fixed left-0 top-0 bottom-0 z-50 w-72 bg-emerald-950 border-r border-emerald-800 flex flex-col overflow-y-auto lg:hidden">
              ...
              return (
                <Link key={item.id} href={href} onClick={() => setMenuOpen(false)}
                  className={cn("flex items-center gap-3 rounded-sm px-3 py-2.5 text-body transition-all",
                    active ? "bg-emerald-900/60 border border-emerald-700 text-primary font-medium" : "text-muted-foreground hover:bg-emerald-900/40 hover:text-foreground"
                  )}>
                  <Icon className="h-[18px] w-[18px]" strokeWidth={active ? 2 : 1.5} />
                  {item.label}
                </Link>
